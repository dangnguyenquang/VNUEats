...
import { ChatRequest, Message } from "@/types/api/chatbot"
...
const Chatbot = () => {
  ...
  const handleSend = async () => {
    ...
    setIsBotTyping(true)
    const history: Message[] = messages.slice(-6).map((msg) => ({
    ...
    try {
      const res = await userAction.chatBot(payload)
  ...
  return (
    <div className='fixed bottom-4 right-4 z-50'>
      {!isOpen && (
        ...
        >
          Mở <strong className='text-secondary'>vnuEats</strong> chatbot
        </button>
      )}
        ...
        <div className='w-80 bg-white shadow-[0_3px_10px_rgb(0,0,0,0.2)] rounded-lg overflow-hidden z-50'>
          ...
          >
            <div className={`text-left max-w-[92%] mb-2`}>
              ...
              >
                Xin chào, mình là trợ lý ảo của{" "}
                <strong className='text-secondary'>vnuEats</strong>, mình sẵn sàng trả lời
                bất kì câu hỏi nào của bạn ^_^!
              </span>
            </div>
