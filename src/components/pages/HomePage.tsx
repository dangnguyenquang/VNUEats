...
import RecommendCarousel from "@/app/(public-routes)/_components/RecommendCarousel"
import SpecialityCarousel from "@/app/(public-routes)/_components/SpecialityCarousel"
...
const HomePage = ({ data }: { data: FoodData | undefined }) => {
  ...
  const handleSubmit = () => {
    router.push(`${PATH_NAME.MENU}?search=${searchValue}&type=${typeSearch}`)
  }
    ...
    <main className='w-full'>
      ...
      >
        <div className='py-10 mx-auto text-center h-full'>
          ...
          <div className='overflow-hidden'>
            {data && <SpecialityCarousel SpecialityFoods={data?.specialtyFoods || []} />}
          </div>
        </div>
        ...
        <div className='w-full mt-14'>
          <div className='relative w-fit ml-auto pl-16 max-lg:overflow-hidden'>
            <img
              ...
              alt='vector'
              className='absolute max-lg:top-10 z-10 bottom-0'
            />
      ...
      >
        ...
        <div className='mx-auto max-w-[1100px] max-md:px-12'>
          {data && <RecommendCarousel restaurants={data?.restaurants || []} />}
        </div>
      </section>
    </main>
  )
