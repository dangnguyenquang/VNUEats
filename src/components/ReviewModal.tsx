...
import { ReviewModalProps } from "@/types/api/order"
...
const ReviewModal: React.FC<ReviewModalProps> = ({ open, onClose, items, idOrder }) => {
  const [ratings, setRatings] = React.useState<number[]>(
    items.map((item) => item.food.starMedium || 0),
  )
