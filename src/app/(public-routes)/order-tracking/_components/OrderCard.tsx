...
import { OrderProps } from "@/types/api/order"
...
const OrderCard: React.FC<OrderProps> = ({ order }) => {
  ...
  const statusMap: Record<string, { label: string; bgColor: string; actions: string[] }> =
    {
  ...
  return (
    <div className='p-4 border rounded-lg w-full sm:w-[600px] my-3 relative bg-white'>
      ...
      <div className='flex flex-col sm:flex-row justify-end space-y-2 sm:space-y-0 sm:space-x-3 mt-3'>
        ...
        <Link href={`${PATH_NAME.DETAILS}/${order.restaurantId}`}>
          <Button
            variant='outline'
